...
const getCount = () => {
      ...
      .catch(error => console.error('Error fetching feedbacks:', error));
        ...
        const handleAddFeedback = () => {
          fetch('http://localhost:3001/api/feedback', {
            ...
            body: JSON.stringify({ comment: newFeedback }),
          })
            .then(response => response.json())
            .then(data => {
  ...
  const handleLike = (id) => {
    fetch(`http://localhost:3001/api/feedback/${id}/like`, {
      method: 'PUT',
    })
      .then(response => response.json())
      .then(data => {
  ...
  const handleDislike = (id) => {
    fetch(`http://localhost:3001/api/feedback/${id}/dislike`, {
      method: 'PUT',
    })
      .then(response => response.json())
      .then(data => {
        console.log('Feedback disliked successfully:', data);
        fetchFeedbacks();
      })
              ...
              <ul className="feedback-list">
                {feedbacks.map(feedback => (
                  <li key={feedback._id} className="feedback-item">
                    <span className="feedback-text">{feedback.comment}</span>
                    <span className="feedback-info"> Likes: {feedback.likes}</span>
                    <span className="feedback-info"> Dislikes: {feedback.dislikes}</span>
                    
                    <div className="feedback-buttons">
                      <Button variant="success" onClick={() => handleLike(feedback._id)}>
                      ...
                      <Button variant="danger" onClick={() => handleDislike(feedback._id)}>
                      ...
                      <Button variant="danger" onClick={() => handleDeleteFeedback(feedback._id)}>
